...
const getUID = async () => {
	if (typeof navigator.connection !== 'undefined' && typeof navigator.connection.getNetworkInformation === 'function') {
	  const networkInfo = await navigator.connection.getNetworkInformation();
	  const macAddress = networkInfo.mac ?? '';
	  return macAddress;
	} else {
	  // fallback to another method to get the UID
	}
  };
  ...
  const submitAddtoCart = (e) => {
	...
	if (!user) {
	  // User is not logged in, get laptop MAC address as uid
	  getUID().then(macAddress => {
		const data = {
		  pid: product.id,
		  pqty: quantity,
		  uid: macAddress, // Use laptop MAC address as uid
		  datetime: date,
		}
